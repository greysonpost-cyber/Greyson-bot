...
function entryButton(g) {
    return new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            .setCustomId(`giveaway_enter:${g.id}`)
...
function claimButton(claimId) {
    return new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            .setCustomId(`giveaway_claim:${claimId}`)
...
function giveawayEmbed(g) {
    return baseEmbed()
        .setTitle(`🎉 ${g.prize}`)
        .setDescription(
            `Click below to enter! Click the button again to get the option to leave.\n\n` +
...
async function refreshGiveawayMessage(client, g) {
    if (!g?.message_id || !g?.channel_id || !g?.guild_id) return;
    const guild = client.guilds.cache.get(g.guild_id) || await client.guilds.fetch(g.guild_id).catch(() => null);
    const channel = guild ? await guild.channels.fetch(g.channel_id).catch(() => null) : null;
    const message = channel ? await channel.messages.fetch(g.message_id).catch(() => null) : null;
...
function autoClaim(member) {
    return db.prepare('SELECT role_id FROM giveaway_auto_claim_roles WHERE guild_id=?')
        .all(member.guild.id)
...
async function createWinnerTicket(guild, g, winnerId) {
    ...
    const info = db.prepare(
        'INSERT OR REPLACE INTO giveaway_claims (giveaway_id,guild_id,winner_id,deadline_at,claimed,auto_claimed,resolved,created_at) VALUES (?,?,?,?,?,?,0,?)'
    ).run(g.id, guild.id, winnerId, deadline, auto ? 1 : 0, auto ? 1 : 0, Date.now());
    ...
    await channel.send({
        content: `<@${winnerId}>${staffRole ? ` <@&${staffRole}>` : ''}`,
        embeds: [infoEmbed(
            auto ? '🎉 Prize Auto-Claimed' : '🎉 You Won!',
            `You won **${g.prize}** in giveaway **#${g.id}**.\n${auto ? 'Your configured role automatically marked this prize claimed.' : `Press **Claim Prize** before <t:${Math.floor(deadline / 1000)}:R> or the bot will reroll.`}`
        )],
        components: auto ? [] : [claimButton(claimId)],
    });
    return channel;
}
...
async function rerollExpired(client, claim) {
    ...
    if (!old) return;
    deleteEntry.run(old.id, claim.winner_id);
    ...
    if (claim.ticket_channel_id && guild) {
        ...
        if (c) {
            await c.send({ embeds: [errorEmbed('Claim Time Expired', 'The prize was not claimed in time. A new winner is being selected.')], components: [] });
...
async function handleInteraction(i, client) {
    ...
    if (action === 'giveaway_enter') {
        ...
        if (existing) {
            return i.reply({
                embeds: [infoEmbed('Already Entered', `You currently have **${existing.entries}** entries. Press below to leave this giveaway.`)],
    ...
    if (action === 'giveaway_leave_confirm') {
        ...
        if (g.ended) return i.update({ embeds: [errorEmbed('Giveaway Ended', 'You can no longer change your entry.')], components: [] });
        const existing = getEntry.get(id, i.user.id);
        ...
        await refreshGiveawayMessage(client, g);
        return i.update({ embeds: [successEmbed('Left Giveaway', 'All of your entries, including bonus entries, were removed.')], components: [] });
    }

    if (action === 'giveaway_claim') {
        const claim = db.prepare('SELECT * FROM giveaway_claims WHERE id=?').get(id);
        if (!claim || claim.winner_id !== i.user.id) return i.reply({ embeds: [errorEmbed('Not Your Claim', 'Only the selected winner can claim this prize.')], ephemeral: true });
        ...
        db.prepare('UPDATE giveaway_claims SET claimed=1,resolved=1 WHERE id=?').run(id);
        return i.update({ embeds: [successEmbed('Prize Claimed!', `<@${i.user.id}> claimed this prize. Staff can now complete delivery in this ticket.`)], components: [] });
    }
}
