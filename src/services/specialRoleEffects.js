...
function roleState(member) {
  ...
  if (collection) return { collection: true, label: collection.collection_name, roleId: collection.role_id };
  ...
  return { collection: false, label: artifact.name, rarity: artifact.rarity, roleId: artifact.discord_role_id };
}
...
async function trigger(channel, member, context, options = {}) {
  ...
  const line = collection ? pick(collectionLines)(name) : pick(normalLines)(name);
  const rarity = collection ? (rareRoll < .03 ? '🌟 MYTHIC 3% VARIANT' : rareRoll < .10 ? '✨ ULTRA-RARE VARIANT' : '🌌 COLLECTION EXCLUSIVE') : (rareRoll < .02 ? '💥 SECRET 2% VARIANT' : rareRoll < .08 ? '✨ RARE VARIANT' : '🎆 LIMITED COSMETIC');
  const palette = collection ? COLLECTION_COLORS : COLORS;
  const embed = new EmbedBuilder()
    .setColor(palette[0])
    .setAuthor({ name: `DRIPCORE • ${contextLabel(context)}` })
    .setTitle(collection ? '🌌 SPIDER-VERSE COLLECTION EFFECT' : `🎇 ${state.label.toUpperCase()} EFFECT`)
    .setDescription(`━━━━━━━━━━━━━━━━━━━━\n${line}\n\n${rarity}\n━━━━━━━━━━━━━━━━━━━━`)
